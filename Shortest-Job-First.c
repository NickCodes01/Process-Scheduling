...
//and now we add to our linkd list via the SJF function call that includes two parameters, address of pointer to front of linkd list and n task
SJF(&front, n);
}
...
//first is our SJF function which sorts by CPU burst
void SJF(LL** front, Hold n) {
...
//traverse list
LL** now = front;
//keep traversing until we locate insertion point based on burst
//compare bursts
while(*now && (*now)->n.burst < n.burst) {
now = &((*now)->linked);
}
